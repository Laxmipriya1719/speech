...
class ConvFeatureExtractionModel(nn.Module):
    def __init__(
            self,
            #conv_layers: list = [(512,10,5)] + [(512,3,2)] * 3 + [(512,2,2)] + [(512,1,1)],
    ...
    def forward(self, x):
        for conv in self.conv_layers:
            if x.shape[-1] < conv[0].kernel_size[0]:  # conv[0] to access actual Conv1D layer
                break  # Or skip this conv layer
            x = conv(x)
            return x
